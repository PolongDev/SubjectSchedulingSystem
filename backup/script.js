...
document.querySelector(".subjectsLink").setAttribute("href", `${url}/sched/config/Subjects`);
document.querySelector(".instructorsLink").setAttribute("href", `${url}/sched/config/Instructors`);

let schedule = [], localdb;

function showPopup2(message, func){
  document.querySelector(".popupBoxInfo2").innerHTML = message;
  document.querySelector('.deleteLabel').setAttribute('onclick', func);
...
async function conflictChecker(sched){
  if(schedule.length > 0){
    const conflict = await isConflictSchedule(sched);
    if (conflict) return conflict;
    schedule.push(sched);
  }else{
    const conflictAll = await isAllSchedConflict(sched);
...
async function actionDelete(event){
  const subject = event.parentElement.parentElement.parentElement.firstElementChild.innerText;
  const confirmed = confirm(`Are you sure you want to delete ${subject} subject?`);
  if (confirmed) {
    try {
      const response = await fetch(`${url}/api/action/deleteSched?label=${labelName}&department=${department}&subject=${subject}`);
      loadSchedules();
    } catch (e) {console.log(e)}
  }
}
...
async function actionEdit(event){
  const subjectName = event.parentElement.parentElement.parentElement.firstElementChild.innerText;
  const response = await fetch(`${url}/api/action/editSched?label=${labelName}&department=${department}&subject=${subjectName}`);
  const data = await response.json();
  ...
  document.querySelector('.saveEditBtn').onclick = async () => {
    ...
    const changedDays = data[0].day.length === newDaySelected.length ? newDaySelected.map((day, i) => day === data[0].day[i]) : [false];
    const possibleConflict = Number(editSHours.value) === data[0].time.start[0] && Number(editSMinutes.value) === data[0].time.start[1] && Number(editEHours.value) === data[0].time.end[0] && Number(editEMinutes.value) === data[0].time.end[1] && editOption.value === data[0].timeStatus && !changedDays.includes(false) && editRoom.value === data[0].room;
    if (!possibleConflict) {
      if (await conflictChecker(newSched)) return;
    }
    
    const response = await fetch(`${url}/api/action/editSched?label=${labelName}&department=${department}&subject=${subjectName}&newSched=${JSON.stringify(newSched)}`);
...
async function loadSchedules(){
  const response = await fetch(`${url}/api/getScheduleData?label=${labelName}&department=${department}.json`);
  const data = await response.json();
    try {
        ...
        for (let i = 0; i < schedule.length; i++) {
          ...
          <tr>
            ...
            <td>${
              ...
              schedule[i].day.length == 2 && schedule[i].day.includes("Monday") && schedule[i].day.includes("Wednesday") ?
              "MW" :
              schedule[i].day.length == 2 && schedule[i].day.includes("Tuesday") && schedule[i].day.includes("Thursday") ?
              "TTH" :
              schedule[i].day.length > 2 && schedule[i].day.includes("Monday") && schedule[i].day.includes("Wednesday") ?
              `MW, ${schedule[i].day.filter((e) => e == "Monday" || e == "Wednesday" ? false : true).map(d => d.slice(0, 3)).join(", ")}` :
              schedule[i].day.length > 2 && schedule[i].day.includes("Tuesday") && schedule[i].day.includes("Thursday") ?
              `TTH, ${schedule[i].day.filter((e) => e == "Tuesday" || e == "Thursday" ? false : true).map(d => d.slice(0, 3)).join(", ")}` :
...
window.onload = async () => {
  ...
  localdb.subjects.map(subj => (
    document.querySelector('#subjects').innerHTML += `<option value="${subj}">${subj}</option>`
  ));
  localdb.instructors.map(instr => (
    document.querySelector('#instructors').innerHTML += `<option value="${instr}">${instr}</option>`
  ));
  localdb.rooms.map(room => (
    document.querySelector('#rooms').innerHTML += `<option value="${room.value}">${room.name}</option>`
  ));
  localdb.rooms.map(room => (
    document.querySelector('.editRoom').innerHTML += `<option value="${room.value}">${room.name}</option>`
  ));
}
  ...
  if(!inputStatus.includes(false)){
    ...
    contents.innerHTML += `
      <tr>
        <td>${subject.value}</td>
        <td>
          ${SHours.value}:${SMinutes.value > 0 ? SMinutes.value : String(SMinutes.value) + "0"} - ${EHours.value}:${EMinutes.value > 0 ? EMinutes.value : String(EMinutes.value) + "0"} ${option.value}
        </td>
        <td>${room.value}</td>
        <td>${
          selectedDay.length == 1 ?
          selectedDay[0].slice(0,3) :
          selectedDay.length == 2 && selectedDay.includes("Monday") && selectedDay.includes("Wednesday") ?
          "MW" :
          selectedDay.length == 2 && selectedDay.includes("Tuesday") && selectedDay.includes("Thursday") ?
          "TTH" :
          selectedDay.length > 2 && selectedDay.includes("Monday") && selectedDay.includes("Wednesday") ?
          `MW, ${selectedDay.filter((e) => e == "Monday" || e == "Wednesday" ? false : true).map(d => d.slice(0, 3)).join(", ")}` :
          selectedDay.length > 2 && selectedDay.includes("Tuesday") && selectedDay.includes("Thursday") ?
          `TTH, ${selectedDay.filter((e) => e == "Tuesday" || e == "Thursday" ? false : true).map(d => d.slice(0, 3)).join(", ")}` :
        ...
        <td class="actionsContainer">
          <div class="actions">
            <button class="editAction" onclick="actionEdit(this)">edit</button>
            <button class="deleteAction" onclick="actionDelete(this)">del</button>
          </div>
        </td>
      </tr>
    `;
    ...
    EHours.value = "";
    EMinutes.value = "";
    for (day of days) day.checked = false;
...
document.querySelector(".deleteBtn").onclick = () => {
    const message = `Are you sure you want to delete <span style="color:red">${decodeURI(department)} schedule</span>?`;
